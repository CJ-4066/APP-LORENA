...
export type AdminCourseResource = {
  id: string;
  courseId: string;
  moduleId?: string | null;
  lessonId?: string | null;
                    ...
                    <div className="library-list">
                      {libraryVisiblePdfs.slice(0, 12).map((pdf) => (
                        <article key={pdf.id} className="library-list-row">
                          <a
                            className="library-list-preview"
                            href={`${apiBaseUrl}/api/content/library/pdfs/${encodeURIComponent(pdf.id)}/view`}
                            target="_blank"
                            rel="noreferrer"
                            aria-label={`Abrir ${pdf.title}`}
                          >
                            <img
                              src={`${apiBaseUrl}/api/content/library/pdfs/${encodeURIComponent(pdf.id)}/pages/1/image?width=320`}
